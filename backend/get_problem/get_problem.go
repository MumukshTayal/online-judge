...
func GetProblemByProblemId(c *fiber.Ctx, problemID int) error {
...
func GetProblemByUserId(c *fiber.Ctx, userId int) ([]int, error) {
	err := godotenv.Load(".env")
	if err != nil {
		log.Fatalf("Some error occured. Err: %s", err)
	}
	...
	rows, err := db.Query("SELECT problem_id FROM problem WHERE creator_id = ?", userId)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
